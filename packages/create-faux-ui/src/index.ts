...
function buildDomApp(name: string): string {
  return [
    'import { createDomTextMeasurer, mountDomRoot, type DomElementLike } from "@faux-ui/dom";',
    'import { createReconciler, Text, View } from "@faux-ui/reconciler";',
    'import "./styles.css";',
    "",
    `const appName = ${JSON.stringify(name)};`,
    ...
    "  if (mountedNode === null) {",
    '    throw new Error("Expected a mounted faux-ui root node.");',
    "  }",
    "",
    "  if (mounted === null) {",
    "    mounted = mountDomRoot<() => void>(mountedNode, {",
