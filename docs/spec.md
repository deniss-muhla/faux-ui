...
This is the normative target for the unreleased refactor. The implementation on `main` predates this specification and is not yet conformant. Delivery status belongs in [roadmap.md](roadmap.md); rationale and evidence belong in [refactor/report.md](refactor/report.md).

Two implementation details must be pinned before the semantic kernel lands:

- Unicode data/version used for grapheme cell widths;
- the minimal accessibility projection of the DOM application surface.

Neither may change layout or interaction semantics by host.
...
- a border contributes one cell on each enabled edge;
...
The implementation pins one Unicode data/version and exposes it to tests/inspection.

The shared cellizer must:

- segment extended grapheme clusters;
- assign deterministic widths of 0, 1, or 2;
- treat East Asian ambiguous width as 1;
- attach zero-width combining clusters to a leading cell when valid;
- reserve a continuation cell for width-2 graphemes;
- define replacement behavior for unsupported controls;
- define tab behavior explicitly (tabs are not host-expanded implicitly).
...
- Wheel, terminal mouse, keyboard commands, and public scroll methods all update the same controller state.
...
- Expose the approved application/action accessibility labels without changing semantics.
